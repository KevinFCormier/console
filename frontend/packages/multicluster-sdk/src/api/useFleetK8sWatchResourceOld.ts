...
import { handleWebsocketEventOld } from '../internal/fleetK8sWatchResourceOld'
import { getCacheKeyOld, useFleetK8sWatchResourceStoreOld } from '../internal/fleetK8sWatchResourceStoreOld'
 ...
 * ```
 */
export function useFleetK8sWatchResourceOld<R extends FleetK8sResourceCommon | FleetK8sResourceCommon[]>(
  ...
  const cachedEntry = useFleetK8sWatchResourceStoreOld((state) => state.resourceCache[requestPath])
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      const socketKey = getCacheKeyOld({ model, cluster, namespace, name })
      ...
      try {
        ...
        socket.onmessage = (event) => {
          try {
            ...
            handleWebsocketEventOld(event, requestPath, isList, cluster)
          } catch (e) {
