...
import { useFleetK8sWatchResourceStoreOld, useFleetK8sCacheOld, getCacheKeyOld } from './fleetK8sWatchResourceStoreOld'
...
describe('FleetK8sWatchResourceStore', () => {
  ...
  it('should set and get resource data', () => {
    const { result } = renderHook(() => useFleetK8sWatchResourceStoreOld())
  ...
  it('should handle resource expiration', async () => {
    ...
    act(() => {
      ...
      useFleetK8sWatchResourceStoreOld.setState({ cacheTTL: 1 })
    })
...
describe('useFleetK8sCache hook', () => {
  it('should provide cache management utilities', () => {
    const { result } = renderHook(() => useFleetK8sCacheOld())
  ...
  it('should allow setting cache TTL', () => {
    ...
    expect(useFleetK8sWatchResourceStoreOld.getState().cacheTTL).toBe(10000)
  })
})
...
describe('Resource cache advanced scenarios', () => {
  ...
  it('should clear expired resources only', () => {
    ...
    // Set very short TTL
    act(() => {
      useFleetK8sWatchResourceStoreOld.setState({ cacheTTL: 50 })
    })
...
describe('getCacheKey utility', () => {
  it('should generate consistent cache keys', () => {
    ...
    const key1 = getCacheKeyOld({ model, cluster, namespace, name })
    const key2 = getCacheKeyOld({ model, cluster, namespace, name })
  ...
  it('should handle optional parameters', () => {
    ...
    const keyWithCluster = getCacheKeyOld({ model, cluster: 'test-cluster' })
    expect(keyWithCluster).toBe('test-cluster||v1|Namespace||')

    const keyMinimal = getCacheKeyOld({ model })
