...
export interface SocketCacheEntryOld {
  socket: WebSocket
  timestamp: number
  refCount: number // Track how many hooks are using this socket
}

export interface FleetK8sWatchResourceStoreOld {
  // Resource cache
  resourceCache: Record<string, CacheEntry>

  // Socket cache
  socketCache: Record<string, SocketCacheEntryOld>
  ...
  getSocket: (key: string) => SocketCacheEntryOld | undefined
...
export const useFleetK8sWatchResourceStoreOld = create<FleetK8sWatchResourceStoreOld>()(
  subscribeWithSelector((set, get) => ({
    ...
    clearExpired: () => {
      ...
      set((state) => {
        ...
        const validSockets: Record<string, SocketCacheEntryOld> = {}
...
export const useFleetK8sCacheOld = () => {
  const store = useFleetK8sWatchResourceStoreOld()

  return {
    clearExpired: store.clearExpired,
    clearAll: store.clearAll,
    setCacheTTL: (ttl: number) => {
      useFleetK8sWatchResourceStoreOld.setState({ cacheTTL: ttl })
    },
  }
}
